...
public class SelectionInteractor2 : GenericInteractor
{
    ...
    public bool LookForNewTarget()
    {
        ...
        var target = ConeCastWithChecks(_op.position, _op.right, _maxRadius, _maxDistance, _coneAngle, _selectableLayerMask, .2f, .3f, .2f, .3f, out _newHoverTargetRaycast);
    ...
    public void GenerateBezierLineRenderer()
    {
        ...
        var forward = _op.right;
        _directions.Add(forward);
    ...
    public bool ConeCastWithChecks(Vector3 origin, Vector3 direction, float maxRadius, float maxDistance, float coneAngle, LayerMask layerMask, float distanceWeight, float angleWeight, float distanceToCenterWeight, float angleToCenterWeight,out RaycastHit raycastHit)
    {
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (_coneCastGenericInteractable == null)
            {
                continue;
            }
            switch (_editingStateReference.Value)
            {
                case EditingState.Spline when !_coneCastGenericInteractable.gameObject.CompareTag("Spline"):
                case EditingState.Object when !_coneCastGenericInteractable.gameObject.CompareTag("Object"):
                    continue;
            }
    ...
    {
        base.OnEnable();
        _originPoint.RegisterListenerExtended(OnOriginCreate, OnOriginDestroy,OnOriginEnabled,OnOriginDisabled);
    }

    private void OnOriginEnabled(Object obj)
    {
        _op = ((GameObject)obj).transform;
        _trackingActive = true;
    }

    private void OnOriginDisabled(Object obj)
    {
        _op = null;
    ...
    {
        base.OnDisable();
        _originPoint.UnregisterListenerExtended(OnOriginCreate, OnOriginDestroy,OnOriginEnabled,OnOriginDisabled);
    }
    #endregion
    ...
    {
        OnConditionsChanged();
        _visualsEnabled = _interactorActive;
        _lineRenderer.enabled = _visualsEnabled;
        if (_visualsEnabled) return;
        if (_newHoverTargetInteractable == null) return;
    ...
    private void SelectCurrentlyHoveredInteractable()
    {
        ...
        if (_currentlySelectedInteractable.Value == _currHoverInter)
        {
            _currentlySelectedInteractable.Value.StopSelect();
            _currentlySelectedInteractable.Value = null;
            return;
        }
    ...
    protected override void OnLeftGestureChanged(Gesture.GestureID gestureID)
    {
        if (_leftExceptMode)
        {
            _leftGestureActive = _leftInteractorGesture.Value != Gesture.GestureID.Hover || _leftInteractorGesture.Value != Gesture.GestureID.Select;
        }
        else
        {
            _leftGestureActive = _leftInteractorGesture.Value == Gesture.GestureID.Hover || _leftInteractorGesture.Value == Gesture.GestureID.Select;
        }
        if ((_handedness == Handedness.Left || _handedness == Handedness.Both) && _leftInteractorGesture.Value == Gesture.GestureID.Select)
        {
    ...
    protected override void OnRightGestureChanged(Gesture.GestureID gestureID)
    {
        if (_rightExceptMode)
        {
            _rightGestureActive = _rightInteractorGesture.Value != Gesture.GestureID.Hover || _rightInteractorGesture.Value != Gesture.GestureID.Select;
        }
        else
        {
            _rightGestureActive = _rightInteractorGesture.Value == Gesture.GestureID.Hover || _rightInteractorGesture.Value == Gesture.GestureID.Select;
        ...
        if ((_handedness == Handedness.Right || _handedness == Handedness.Both) && _rightInteractorGesture.Value == Gesture.GestureID.Select)
        {
            SelectCurrentlyHoveredInteractable();
        }
        OnConditionsChanged();
