...
public class DynamicGestureValidator : MonoBehaviour
{
    ...
    [SerializeField] private Detection _currentGesture = new Detection(Gesture.GestureName.None,0f);
    [SerializeField] private Detection _lastGesture = new Detection(Gesture.GestureName.None,0f);
    ...
    {
        [FormerlySerializedAs("gestureID")] public Gesture.GestureName gestureName;
        [Range(0f,1f)] public float confidence;
        public Detection(Gesture.GestureName gestureName, float confidence)
        {
            this.gestureName = gestureName;
            this.confidence = confidence;
        }
    }
    private static readonly List<string> GestureIDList = Enum.GetNames(typeof(Gesture.GestureName)).ToList();
    ...
    private void DecideOnCurrentGesture()
    {
        ...
        {
            if (_currentGesture.gestureName != _lastGesture.gestureName)
            {
                _gestureIdReference.Value = _currentGesture.gestureName;
                _lastGesture = _currentGesture;
            }
            return;
        }
        _currentGesture = currentGestureScores.First(e => e.gestureName == _currentGesture.gestureName);
        ...
        _currentGesture = new Detection(Gesture.GestureName.None, 0f);
        var highestConfidence = new Detection(Gesture.GestureName.None, 0f);
        ...
        if (highestConfidence.confidence <= gestureInitializationReq)
        {
            ...
            _gestureIdReference.Value = Gesture.GestureName.None;
            return;
        }
    ...
    public void PushRemoteDetectionToQueue(float[] detections)
    {
        for (int i = 0; i < GestureIDList.Count; i++)
        {
            ...
            _detectionFrame[i].Insert(0,new Detection((Gesture.GestureName) i, detections[i]));
    ...
    private void PushNewDetectionToQueue()
    {
        for (int i = 1; i < GestureIDList.Count; i++)
        {
            ...
            _detectionFrame[i].Insert(0,new Detection((Gesture.GestureName) i, CycleConfidenceValues(i-1)));
    ...
    private void CalculateGestureConfidence()
    {
        ...
        for (int i = 0; i < GestureIDList.Count; i++)
        {
            ...
            currentGestureScores.Add(new Detection((Gesture.GestureName)i, confidence));
        }
